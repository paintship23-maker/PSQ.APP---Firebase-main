...
function calcExteriorConfiguredTotals(exterior, globalConfig, quoteMode, paintingType = "fresh") {
  ...
  const elevations = els.map(el => {
    ...
    const resolved = resolveExteriorConfig(el, gc);
    ...
    <div style={{marginBottom:6}}>
      ...
      <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
        {project.floors.map((fl,fi)=>{
          const abbr=(fl.name||'').toString().replace("Ground","GF").replace("First","FF").replace("Second","SF").replace("Third","TF").replace("Fourth","4F").replace(" Floor","").replace("Floor","").trim()||fl.name.slice(0,3);
...
function generatePDF(project) {
  ...
  const savedLogoSrc = (typeof localStorage !== "undefined" && localStorage.getItem("custom_pdf_logo")) || "/Paintship%20W-W-Logo%20(5).png";
  ...
  const logoSrc = (typeof localStorage !== "undefined" && localStorage.getItem("custom_pdf_logo")) || "/Paintship%20W-W-Logo%20(5).png";
...
export default function App() {
  ...
  useEffect(()=>{ if(user) loadAllProjects().then(projects => {
    // Ensure all rooms have finishing object (fix for undefined room.finishing)
    const migrated = projects.map(p => ({
      ...
      floors: (p.floors || []).map(fl => ({
        ...
        rooms: (fl.rooms || []).map(r => ({
          ...r,
          // Ensure finishing exists with default values if missing
          finishing: r.finishing || defFinishing(r.package || "premium")
        }))
      }))
    }));

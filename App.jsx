...
function mergeCustomFinishTypes(baseMeta, resolveCategoryKey) {
  try {
    ...
    for (const [subKey, cfg] of Object.entries(baseMeta)) {
      ...
      // Standard tiers, then any user-added tiers from Master Rates, then the
      // synthetic "type your own name" option (if this sub-category has one) —
      // same order the old hardcoded {id:"custom"} placeholder used to give.
...
function defFinishing(pkg = "premium") {
  ...
  return {
    putty:    { on:true,  type:"white_cement",    customName:"", rate:p.putty,   coats:2, area:0, useRoom:true,  consumption:defConsumptionObj("putty")    },
    primer:   { on:true,  type:"interior",         customName:"", rate:p.primer,  coats:1, area:0, useRoom:true,  consumption:defConsumptionObj("primer")   },
    paint:    { on:true,  type:"economy_emulsion", customName:"", rate:p.paint,   coats:2, area:0, useRoom:true,  consumption:defConsumptionObj("paint")    },
    topcoat:  { on:false, type:"clear_varnish",    customName:"", rate:p.topcoat, coats:1, area:0, useRoom:true,  consumption:defConsumptionObj("topcoat")  },
...
function defExteriorFinishing(pkg = "premium") {
  const p = PACKAGES[pkg];
  return {
    putty:      { on:true,  type:"exterior_putty",   customName:"", rate:12,       coats:2, area:0, useRoom:true },
    primer:     { on:true,  type:"exterior_primer",  customName:"", rate:p.primer, coats:1, area:0, useRoom:true },
    paint:      { on:true,  type:"premium_ext",      customName:"", rate:p.paint,  coats:2, area:0, useRoom:true },
    protection: { on:false, type:"waterproof",       customName:"", rate:18,       coats:1, area:0, useRoom:true },
    texture:    { on:false, type:"exterior_texture", customName:"", rate:35,       coats:1, area:0, useRoom:false },
  };
}
function newOpening(kind, mode="deduct") { return { id:uid(), kind, label:kind, w:0, h:0, count:1, mode, notes:"" }; }
function newExtraWall(mode) { return { id:uid(), label:mode==="add"?"Extra Wall":"Deduction", w:0, h:0, mode }; }
...
function newSegment(kind="flat") {
  return { id:uid(), kind, label:"", w:0, h:null, depth:0, openings:[] };
}
function newSegOpening(kind="Door", mode="deduct") {
  return { id:uid(), kind, label:kind, mode, w:0, h:0, count:1, notes:"" };
}
...
function calcSegArea(seg, effH) {
  ...
  if (seg.depth > 0) {
    if (seg.kind === "recess" || seg.kind === "projection") {
      ...
      gross += 2 * (seg.depth||0) * sh + (seg.w||0) * (seg.depth||0); // 2 sides + back
    }
  }
  const opAdj = (seg.openings||[]).reduce((s,o)=>{
    const a = (o.w||0)*(o.h||0)*(o.count||1);
...
function calcWallArea(wall, roomHeight) {
  const effH = wall.height ?? roomHeight ?? 10;
  return (wall.segments||[]).reduce((s,seg)=>s+calcSegArea(seg,effH), 0);
}
...
function newRoom(type, pkg, brand) {
  return { id:uid(), type, customType:"", package:pkg, brand, customBrand:"",
    ...
    finishing:defFinishing(pkg), labourRate:PACKAGES[pkg].labour, labourRateExcl:PACKAGES[pkg].labourExcl,
...
function defExterior() { return ELEVATIONS.map(name => ({ id:uid(), name, sections:[{id:uid(),label:"",w:0,h:0}], deductions:[], additions:[], condition:"Good", conditionIssues:[], conditionNotes:"", conditionPhotos:[], exteriorOverride:defExteriorOverride() })); }
function newCondPhoto(image, label) { return { id:uid(), image, label:label||"", timestamp:new Date().toISOString() }; }
function newExtSection() { return { id:uid(), label:"", w:0, h:0 }; }
function newExtDeduction(kind) { return { id:uid(), kind, label:kind, w:0, h:0, qty:1 }; }
function newExtAddition(kind)  { return { id:uid(), kind, label:kind, w:0, h:0, qty:1 }; }
function migrateElevation(el) {
  if (el.sections) return el; // already new format
  return { ...el, sections:[{ id:uid(), label:"", w:el.w||0, h:el.h||0 }] };
}
function newDWItem() { return { id:uid(), kind:"Wooden Door", label:"Wooden Door", w:0, h:0, qty:1, finish:"oil_paint", customFinish:"", rate:25, coats:2, labourRate:15 }; }
function newProject(user) {
  return { id:uid(), supervisorId:user.cardId, supervisorName:user.name,
...
function calcFinCost(fin, net) {
  return Object.entries(fin).reduce((s,[k,f]) => {
    ...
    return s + (f.rate||0)*(f.coats||1)*a + (k==="wallpaper" ? (f.installRate||0)*a : 0);
  }, 0);
}
function calcRoom(r) {
  const net = calcNet(r);
  const mat = calcFinCost(r.finishing || defFinishing(r.package), net);
  let lab, labEx;
...
function projectTotals(p) {
  ...
  p.floors.forEach(fl => fl.rooms.forEach(r => { const c=calcRoom(r); net+=c.net; mat+=c.mat; lab+=c.lab; labEx+=c.labEx; }));
  return { net, mat, lab, labEx, totalIncl:mat+lab, totalExcl:labEx };
}
function calcExteriorMaterialCost(finishing, netArea) {
  return Object.values(finishing||{}).reduce((s,f) => { if(!f.on) return s; const a=f.useRoom?netArea:(f.area||0); return s+(f.rate||0)*(f.coats||1)*a; }, 0);
}
...
function calcExteriorConfiguredTotals(exterior, globalConfig, quoteMode) {
  ...
  // Split a finishing object into two same-shaped copies: one where only useRoom:true
  // layers stay "on" (everything else forced off), and its mirror for useRoom:false.
  // calcExteriorMaterialCost already skips any layer with on:false, so this reuses that
  // existing behavior instead of adding a second on/off check of our own. Never mutates
  // the source finishing object or its layer objects.
  ...
  const globalManualMaterial = calcExteriorMaterialCost(globalManual, 0); // area unused when useRoom:false
  ...
  const elevations = els.map(el => {
    ...
    const resolved = resolveExteriorConfig(el, gc); // existing helper, not recreated
    const ov = el && el.exteriorOverride;
    const useGlobal = !ov || ov.useGlobal !== false; // identical fail-safe check to resolveExteriorConfig's own

    // Material
    let elMaterial = 0;
    if (withMat) {
      const { auto, manual } = splitFinishing(resolved.finishing);
      elMaterial += calcExteriorMaterialCost(auto, elArea); // per-elevation auto-area layers — correct as-is
      if (!useGlobal) {
        elMaterial += calcExteriorMaterialCost(manual, 0); // override: isolated to this elevation
      } else if (!globalManualCharged) {
        elMaterial += globalManualMaterial; // global manual layer(s): charge once, first eligible elevation
...
function PaintShipLogo({ size=72 }) {
  // Official PaintShip mark, cropped from the full lockup in
  // public/Paintship W-Logo.png (icon sits in the top y:[0,642] band,
  // x:[660,1802] of the 2462x1178 source). CSS background crop only —
  // no new image asset created.
  const iw=2462, ih=1178, x0=660, y0=0, x1=1802, y1=642;
  const tileW=x1-x0, tileH=y1-y0;
  const scale = size / tileH;
  const width = tileW*scale;
  const bgW = iw*scale, bgH = ih*scale;
  const bgX = -x0*scale, bgY = -y0*scale;
  return (
    <div style={{
      width, height:size, flexShrink:0,
      backgroundImage:`url("/Paintship W-Logo.png")`,
...
// ─── CONSUMPTION PANEL ────────────────────────────────────────────
function ConsumptionPanel({ f, net, onChange }) {
  ...
  return <div style={{marginTop:12,background:"#F0F9FF",borderRadius:10,padding:"12px 14px",border:`1px solid ${C.blue}33`}}>
    ...
    {c.enabled&&<>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:8}}>
        <div><span style={LBL}>COVERAGE (sf/L)</span><NumInp small value={c.coverage} onChange={v=>upC("coverage",v)}/></div>
        <div><span style={LBL}>WASTAGE (%)</span><NumInp small value={c.wastage} onChange={v=>upC("wastage",v)}/></div>
        <div><span style={LBL}>PACK SIZE (L)</span><NumInp small value={c.packSize} onChange={v=>upC("packSize",v)}/></div>
        <div><span style={LBL}>RATE (₹/L)</span><NumInp small prefix="₹" value={c.ratePerL} onChange={v=>upC("ratePerL",v)}/></div>
      </div>
      <div style={{background:C.white,borderRadius:8,padding:"8px 10px",marginBottom:8,border:`1px solid ${C.border}`}}>
        ...
        <div style={{fontSize:11,color:"#555"}}>({area.toFixed(1)} sf × {f.coats||1}) ÷ {c.coverage} = <b>{auto.litres} L</b> + {c.wastage}% = <b style={{color:C.blue}}>{auto.litresWithWaste} L</b></div>
      </div>
      ...
      {c.overrideLitres&&<div style={{marginBottom:8}}><span style={LBL}>MANUAL LITRES</span><NumInp small value={c.manualLitres||0} onChange={v=>upC("manualLitres",v)}/></div>}
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6,marginTop:6}}>
        {[["Total Litres",`${fmt(displayL)} L`,C.blue],["Packs Req.",`${displayPacks} packs`,C.purple],["Mat. Cost",`₹${fmt(displayCost)}`,C.orange]].map(([label,val,col])=>(
...
function MeasurementHeader({ project, floor, room, af, ar, setAf, setAr, upRoom, withMat, inr, calcRoom, calcNet, addRoom, addFloor }) {
  return <>
    ...
    <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:7,padding:"6px 10px",
      ...
      <span style={{fontSize:12,fontWeight:800,color:"#fff"}}>{floor.name}</span>
      <span style={{fontSize:12,color:"rgba(255,255,255,0.4)"}}>→</span>
      <span style={{fontSize:12,fontWeight:800,color:C.gold}}>
        {room.type==="Custom"?(room.customType||"Custom"):room.type}
      </span>
    </div>
    ...
    <div style={{marginBottom:6}}>
      ...
      <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
        {project.floors.map((fl,fi)=>{
          const abbr=fl.name.replace("Ground","GF").replace("First","FF").replace("Second","SF").replace("Third","TF").replace("Fourth","4F").replace(" Floor","").replace("Floor","").trim()||fl.name.slice(0,3);
          return <button key={fl.id} onClick={()=>{setAf(fi);setAr(0);}}
    ...
    <div style={{marginBottom:8}}>
      ...
      <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
        {floor.rooms.map((r,ri)=>{
          ...
          return <button key={r.id} onClick={()=>setAr(ri)}
...
function SegOpCard({ op, onUpdate, onRemove }) {
  ...
  return <div style={{background:accentL,borderRadius:10,padding:"10px 12px",marginBottom:6,border:`1.5px solid ${accent}33`}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 16px 1fr 90px",alignItems:"end",gap:6,marginBottom:6}}>
      <div><div style={{fontSize:9,color:"#aaa",fontWeight:700,marginBottom:3}}>W (ft)</div><NumInp small value={op.w} onChange={v=>onUpdate({...op,w:v})}/></div>
      <div style={{textAlign:"center",fontSize:15,color:"#ccc",fontWeight:700,paddingBottom:7}}>×</div>
      <div><div style={{fontSize:9,color:"#aaa",fontWeight:700,marginBottom:3}}>H (ft)</div><NumInp small value={op.h} onChange={v=>onUpdate({...op,h:v})}/></div>
      <div>
    ...
    <div style={{display:"flex",justifyContent:"space-between",background:"#fff",borderRadius:7,padding:"5px 10px",border:`1px solid ${accent}22`}}>
      <span style={{fontSize:10,color:"#aaa"}}>{(op.w||0).toFixed(1)} × {(op.h||0).toFixed(1)} × {op.count||1}</span>
...
function WallViz({ wall, roomHeight, activeSegIdx, onSegClick }) {
  ...
  const totalW = segs.reduce((s,sg)=>s+(sg.w||0),0)||1;
...
function WallEditor({ wall, roomHeight, useRoomHeight, onUpdate }) {
  ...
  const segArea = seg ? calcSegArea(seg, effH) : 0;
  ...
  return <div>
    ...
    {!showAdvanced && seg && <div>
      {useRoomHeight ? (
        <>
          <div style={{marginBottom:6}}>
            ...
            <NumInp value={seg.w} onChange={v=>upSeg(activeSeg,{w:v})}/>
          </div>
      ...
      {/* Area preview */}
      {(!seg.w || seg.w<=0) ? (
        <div style={{background:"#FFF7E6",border:`1px solid ${C.orange}44`,borderRadius:10,
      ...
      {!showAdvOptions ? (
        ...
        <div style={{marginBottom:10}}>
          <button onClick={()=>setShowAdvOptions(false)}
            style={{display:"flex",alignItems:"center",gap:6,padding:"6px 0",border:"none",background:"transparent",
          ...
          <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:8}}>
            {SEG_OPEN_KINDS.map(kind=>(
              <button key={kind} onClick={()=>addSegOp(kind,"deduct")}
                style={{padding:"9px 12px",borderRadius:20,fontSize:11,fontWeight:700,cursor:"pointer",
          ...
          <button onClick={()=>setShowAdvanced(true)}
            style={{width:"100%",padding:"12px 0",borderRadius:10,fontSize:12,fontWeight:700,cursor:"pointer",
    ...
    {showAdvanced && <div>
      ...
      <div style={{display:"flex",gap:5,overflowX:"auto",paddingBottom:4,marginBottom:10}}>
        {segs.map((sg,idx)=>{
          ...
          return <button key={sg.id} onClick={()=>setActiveSeg(idx)}
            style={{flexShrink:0,padding:"8px 12px",borderRadius:20,fontSize:11,fontWeight:700,cursor:"pointer",
            ...
            <span style={{fontSize:9,color:sel?col:"#bbb",marginLeft:4}}>{calcSegArea(sg,effH).toFixed(1)}sf</span>
          </button>;
        })}
      </div>
        ...
        <div style={{display:"flex",gap:5,flexWrap:"wrap"}}>
          {SEG_KINDS.map(sk=>(
            <button key={sk.id} onClick={()=>addSeg(sk.id)}
              style={{padding:"8px 12px",borderRadius:20,fontSize:11,fontWeight:700,cursor:"pointer",
      ...
      {seg&&<div style={{background:"#F8FAFC",borderRadius:12,padding:"12px 14px",marginBottom:10,
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 22px 1fr",alignItems:"end",gap:6,marginBottom:needsDepth?8:10}}>
          ...
          <div>
            ...
            <NumInp value={seg.h??""} placeholder={`${effH} (wall default)`}
              onChange={v=>upSeg(activeSeg,{h:v||null})}/>
          </div>
        </div>

        {/* Width=0 helper, even in advanced mode */}
        {(!seg.w || seg.w<=0) && <div style={{background:"#FFF7E6",border:`1px solid ${C.orange}44`,borderRadius:10,
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",
          ...
          <span style={{fontSize:11,color:"#555",fontWeight:600}}>
            {seg.w||0}ft × {seg.h??effH}ft
            {seg.depth>0?` + ${seg.depth}ft depth`:""}
...
const ITEM_IMAGE_MAP = {
  ...
  "Other Woodwork":  { rect:[1338,0,1536,341.3333] }, // carved door half-tile
...
function ItemPreviewImage({ itemType, size=64, radius=12, fallback }) {
  ...
  if (!m) return <span style={{fontSize:Math.round(size*0.5)}}>{fallback}</span>;
  const [x0,y0,x1,y1] = m.rect || [m.col*ITEM_IMAGE_GRID.cellW, m.row*ITEM_IMAGE_GRID.cellH, (m.col+1)*ITEM_IMAGE_GRID.cellW, (m.row+1)*ITEM_IMAGE_GRID.cellH];
  const tileW = x1-x0, tileH = y1-y0;
  const scale = size / Math.min(tileW, tileH);
  const bgW = ITEM_IMAGE_GRID.naturalW*scale, bgH = ITEM_IMAGE_GRID.naturalH*scale;
...
function calcDoorWindowItem(it) {
  const area = (Number(it.length) || 0) * (Number(it.height) || 0) * (Number(it.qty) || 0);
  ...
  return (
    <div style={{ fontFamily: "inherit" }}>
      ...
      {list.map((it) => {
        ...
        return (
          <div key={it.id} style={{...PCARD,padding:0,overflow:"hidden",border:`1px solid ${isOpen?C.navy:C.border}`}}>
                  ...
                  return (<>
                    <div style={{display:"flex",flexWrap:"wrap",gap:12,marginBottom:20,alignItems:"stretch"}}>
                      ...
                      <div style={{flex:"1 1 160px",minWidth:160,background:C.white,border:`1px solid ${C.border}`,borderRadius:16,padding:"10px 12px",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"space-between",boxShadow:"0 1px 2px rgba(15,30,60,0.05), 0 6px 16px rgba(15,30,60,0.06)"}}>
                        ...
                        <div style={{width:96,height:96,borderRadius:16,background:preview.previewBackground,border:`1.5px solid ${preview.previewBorder}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:28,marginBottom:6,overflow:"hidden"}}><FinishPreviewImage finishId={it.finishType} size={96} radius={14.5} fallback={preview.previewIcon}/></div>
...
function calcWallpaperItem(it) {
  const area = (Number(it.width) || 0) * (Number(it.height) || 0) * (Number(it.qty) || 0);
  const rollArea = (Number(it.rollWidth) || 0) * (Number(it.rollLength) || 0);
  const areaWithWastage = area * (1 + (Number(it.wastage) || 0) / 100);
  const requiredRolls = rollArea > 0 ? Math.ceil(areaWithWastage / rollArea) : 0;
  ...
  return { area, rollArea, areaWithWastage, requiredRolls, materialCost, labourCost, total };
}
...
function buildMaterialConsumptionSummary(project) {
  ...
  // per-room Material Consumption panel already uses. Brand = room's own
  // brand selection; Product = the same getProductName() the room summary/
  // PDF already use for that brand+package.
  (project.floors||[]).forEach(fl=>{
    (fl.rooms||[]).forEach(r=>{
      const net = calcNet(r);
      const fin = r.finishing || defFinishing(r.package);
      const product = getProductName(r.brand, r.package, "interior");
  ...
  (project.exterior||[]).forEach(el=>{
    ...
    const product = getProductName(cfg.brand, cfg.package, "exterior");
    Object.entries(fin).forEach(([key,f])=>{
      if (!f || !f.on || !f.consumption) return;
      const area = f.useRoom ? net : (f.area||0);
      const c = f.consumption;
  ...
  // POLISH_CONSUMPTION_DEFAULTS). Brand/Product read straight off the item
  // (it.brand/it.product — the same fields the item's own Brand/Product
  // dropdowns write to).
...
function MaterialConsumptionSummary({ project }) {
  ...
  return <div style={CARD}>
    ...
    <div style={{border:`1px solid ${C.border}`,borderRadius:12,overflow:"hidden",marginBottom:10}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"0.9fr 1.2fr 0.8fr 0.9fr 0.7fr 0.7fr 0.7fr",minWidth:520,background:"#F8FAFC",padding:"6px 8px",fontSize:8.5,fontWeight:800,color:C.gray,textTransform:"uppercase",letterSpacing:"0.03em"}}>
        <span>Category</span><span>Material</span><span>Brand</span><span>Product/Variant</span>
        <span style={{textAlign:"right"}}>Area (sf)</span><span style={{textAlign:"right"}}>Coverage</span><span style={{textAlign:"right"}}>Est. Qty</span>
      </div>
      {rows.map((r,i)=>(
        <div key={i} style={{display:"grid",gridTemplateColumns:"0.9fr 1.2fr 0.8fr 0.9fr 0.7fr 0.7fr 0.7fr",minWidth:520,padding:"7px 8px",fontSize:10.5,borderTop:`1px solid ${C.border}`,alignItems:"center"}}>
          <span style={{fontSize:9,fontWeight:700,color:C.orange}}>{r.category}</span>
          ...
          <span style={{textAlign:"right",color:"#555"}}>{r.area.toFixed(1)}</span>
          <span style={{textAlign:"right",color:"#555"}}>{r.coverage?`${r.coverage} sf/L`:"—"}</span>
          <span style={{textAlign:"right",fontWeight:700,color:C.orange}}>{r.qty.toFixed(r.unit==="rolls"?0:2)} {r.unit}</span>
        </div>
      ))}
      </div>
    </div>
    ...
    {/* Purchase summary */}
    <div style={{background:"#F8FAFC",borderRadius:10,padding:"8px 12px",display:"flex",flexWrap:"wrap",gap:10,justifyContent:"space-between",alignItems:"center"}}>
      <div style={{display:"flex",gap:14,flexWrap:"wrap"}}>
        <span style={{fontSize:10.5,color:"#555"}}><b style={{color:C.navy}}>{rows.length}</b> unique products</span>
        <span style={{fontSize:10.5,color:"#555"}}><b style={{color:C.navy}}>{rawCount}</b> material rows aggregated</span>
      </div>
      <span style={{fontSize:12,fontWeight:800,color:C.orange}}>
        {grandLitres>0 && `${grandLitres.toFixed(2)} L`}{grandLitres>0&&grandRolls>0&&" · "}{grandRolls>0 && `${grandRolls.toFixed(0)} rolls`}
      </span>
    </div>

    <button onClick={()=>generateMaterialConsumptionPDF(project)}
      style={{width:"100%",marginTop:8,padding:"12px 0",minHeight:44,background:C.navy,color:"#fff",border:"none",borderRadius:12,fontSize:12.5,fontWeight:700,cursor:"pointer"}}>
...
function getProjectServiceTotals(project) {
  ...
  // single global-call calculation when no elevation has an override — see validation).
  const extConfigured = calcExteriorConfiguredTotals(project.exterior || [], project.exteriorConfig || defExteriorConfig(), project.quoteMode);
  ...
  // Grand Total Area = Interior + Exterior only (paintable wall/elevation area).
  // Wallpaper/Texture/Polish areas are NOT added here.
  const grandArea  = interior.area + exterior.area;
...
function hasLegacyExtras(project) {
  ...
  return {
    hasLegacy: legacyDoorCount > 0 || legacyWallpaperCount > 0 || legacyTextureCount > 0,
    legacyDoorCount,
              ...
              return finalGroups.filter(g=>g.items.length>0).map(group=>(
                <details key={keyPrefix+group.label} open={group.items.length===1} style={{border:`1px solid ${C.border}`,borderRadius:12,overflow:"hidden"}}>
                  ...
                  <div style={{display:"flex",flexDirection:"column",gap:10,padding:"10px 10px"}}>
                    {group.items.map((r)=>(
          <div key={r.source+r.id} style={{display:"flex",alignItems:"center",gap:16,padding:"13px 14px",background:C.white,border:`1px solid ${C.border}`,borderRadius:12,boxShadow:"0 1px 2px rgba(15,30,60,0.04), 0 2px 6px rgba(15,30,60,0.04)",minHeight:74}}>
...
function RoomEditor({ room, onUpdate, quoteMode, category }) {
  ...
  const wallAdd=(r.extraWalls||[]).filter(e=>e.mode==="add").reduce((s,e)=>s+(e.w||0)*(e.h||0),0);
  const wallSub=(r.extraWalls||[]).filter(e=>e.mode==="subtract").reduce((s,e)=>s+(e.w||0)*(e.h||0),0);
  const openTotal=(r.openings||[]).reduce((s,o)=>{const a=(o.w||0)*(o.h||0)*(o.count||1);return s+(o.mode==="add"?-a:a);},0);

  // ── Accordion section definitions ──
  const rh = r.roomHeight ?? 10;
  const wallsArea = r.walls.reduce((s,w)=>s+calcWallArea(w,rh), 0);
  const ceilArea=(r.ceiling.l||0)*(r.ceiling.w||0);
  const SECTIONS=[
    ...
    { id:"cond",    icon:"🔍", label:"Condition",   badge:r.condition,                      badgeColor:r.condition==="Good"?C.green:C.orange },
  ];
  ...
  const wallsStepDone = r.walls.some(w=>(w.segments||[]).some(s=>(s.w||0)>0) && calcWallArea(w,rh)>0);
  ...
  return <div>
    ...
    <div style={{borderRadius:14,border:`1px solid ${C.border}`,overflow:"hidden",marginBottom:8}}>
      ...
      {(()=>{
        ...
        const activeWall = r.walls[aw] || r.walls[0];
            ...
            <div style={{marginBottom:10}}>
              ...
              <div style={{display:"flex",gap:6,overflowX:"auto",paddingBottom:4}}>
                {r.walls.map((w,wi)=>{
                  ...
                  const sel=wi===aw;
                  return <button key={w.id} onClick={()=>setAw(wi)}
                    style={{flexShrink:0,padding:"10px 12px",borderRadius:10,fontSize:11,fontWeight:700,
                ...
                <button onClick={addWall}
                  style={{flexShrink:0,padding:"7px 14px",borderRadius:10,fontSize:12,fontWeight:700,
            ...
            {activeWall&&<div style={{borderTop:`1px solid ${C.border}`,paddingTop:14}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
                ...
                {r.walls.length>1&&<button onClick={()=>remWall(aw)}
                  style={{background:C.redL,border:"none",borderRadius:8,padding:"5px 12px",
                    color:C.red,cursor:"pointer",fontSize:11,fontWeight:700}}>Remove Wall</button>}
              </div>
              <WallEditor
                wall={activeWall}
      ...
      {(()=>{
        ...
        return <div style={{borderBottom:`1px solid ${C.border}`}}>
          {isOpen&&<div style={{padding:"12px 14px 14px",background:C.white}}>
            ...
            {r.ceiling.on&&<>
              <DimRow wVal={r.ceiling.l} hVal={r.ceiling.w} onW={v=>upC("l",v)} onH={v=>upC("w",v)} wLabel="LENGTH (ft)" hLabel="WIDTH (ft)"/>
      ...
      {(()=>{
        ...
        const ded=(r.openings||[]).filter(o=>o.mode!=="add").reduce((s,o)=>s+(o.w||0)*(o.h||0)*(o.count||1),0);
        const add=(r.openings||[]).filter(o=>o.mode==="add").reduce((s,o)=>s+(o.w||0)*(o.h||0)*(o.count||1),0);
      ...
      {(()=>{
        ...
        return <div style={{borderBottom:`1px solid ${C.border}`}}>
          {isOpen&&<div style={{padding:"12px 14px 14px",background:C.white}}>
            ...
            {(r.extraWalls||[]).map(ew=>(
              <div key={ew.id} style={{background:"#FAFAFA",borderRadius:12,padding:"12px 14px",marginBottom:8,border:`2px solid ${ew.mode==="add"?C.green+"44":C.red+"44"}`}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
                  <div style={{display:"flex",alignItems:"center",gap:8}}>
                    ...
                    <input value={ew.label} onChange={e=>upEW(ew.id,"label",e.target.value)}
                      style={{fontSize:12,fontWeight:700,color:C.navy,border:`1px solid ${C.border}`,
                        borderRadius:8,padding:"6px 10px",background:C.white,outline:"none",width:100}}/>
                  </div>
                  <div style={{display:"flex",alignItems:"center",gap:6}}>
                    <span style={{fontSize:12,color:ew.mode==="add"?C.green:C.red,fontWeight:700}}>{ew.mode==="add"?"+":"-"}{fmt((ew.w||0)*(ew.h||0))} sf</span>
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 20px 1fr",alignItems:"end",gap:8}}>
                  <div><div style={{fontSize:9,color:"#aaa",fontWeight:700,marginBottom:4}}>W (ft)</div><NumInp small value={ew.w} onChange={v=>upEW(ew.id,"w",v)}/></div>
                  <div style={{textAlign:"center",fontSize:16,color:"#ccc",fontWeight:700,paddingBottom:8}}>×</div>
                  <div><div style={{fontSize:9,color:"#aaa",fontWeight:700,marginBottom:4}}>H (ft)</div><NumInp small value={ew.h} onChange={v=>upEW(ew.id,"h",v)}/></div>
                </div>
              </div>
            ))}
          </div>}
          ...
          {isOpen&&<div style={{padding:"12px 14px 14px",background:C.white}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}>
              {WALL_CONDS.map(cnd=>{
                ...
                return <button key={cnd} onClick={()=>onUpdate({...room,condition:cnd})}
                  style={{padding:"12px 10px",fontSize:12,fontWeight:700,borderRadius:12,textAlign:"left",
            ...
            {r.condition!=="Good"&&<>
              <Inp label="Condition Notes" value={r.conditionNotes||""} onChange={v=>onUpdate({...room,conditionNotes:v})} placeholder="Describe the condition..." rows={3}/>
            ...
            <CondPhotoManager photos={r.conditionPhotos||[]} onChange={v=>onUpdate({...room,conditionPhotos:v})} placeholder="e.g. North Wall Crack, Ceiling Dampness"/>
          </div>}
        </div>;
      })()}
...
function ExteriorMaterialPanel({ config, onChange, quoteMode, extNet }) {
  ...
  const finishing=config.finishing||defExteriorFinishing(config.package);
  const matCost=withMat?calcExteriorMaterialCost(finishing,extNet||0):0;
  const applyPkg=v=>onChange({...config,package:v,finishing:defExteriorFinishing(v),labourRate:PACKAGES[v].labour,labourRateExcl:PACKAGES[v].labourExcl});
  ...
  return <div style={{marginBottom:16}}>
    ...
    <div style={PCARD}>
      ...
      {showFinishing&&<div style={{marginTop:22}}>
        <ExteriorFinishingModule finishing={finishing} onChange={fin=>onChange({...config,finishing:fin})} net={extNet||0}/>
      </div>}
    </div>
...
function ExteriorModule({ elevations, onChange, config, onConfigChange, quoteMode }) {
...
function AdminProjectDetail({ p, onClose }) {
  ...
  return <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",zIndex:600,display:"flex",alignItems:"flex-end",justifyContent:"center"}} onClick={onClose}>
    <div onClick={e=>e.stopPropagation()} style={{background:C.white,borderRadius:"20px 20px 0 0",padding:"20px 16px 36px",width:"100%",maxWidth:480,maxHeight:"92vh",overflowY:"auto"}}>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
        <div>
          <div style={{fontSize:18,fontWeight:800,color:C.navy}}>{p.clientName||p.customer?.name||"Unnamed"}</div>
      ...
      {/* Interior rooms */}
      {showInt&&p.floors?.map(fl=>(
        <div key={fl.id} style={{marginBottom:12}}>
          ...
          {fl.rooms.map(r=>{ const rc=calcRoom(r); const pkg=PACKAGES[r.package]||PACKAGES.premium; const pName=wm?getProductName(r.brand,r.package,"interior"):"";
            return <div key={r.id} style={{background:"#FAFAFA",borderRadius:10,padding:"10px 12px",marginBottom:6,border:`1px solid ${pkg.color}22`}}>
        ...
        :filtered.map(p=>{
          ...
          return <div key={p.id} onClick={()=>setDetailProject(p)}
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
              <div style={{flex:1}}>
                <div style={{fontSize:14,fontWeight:800,color:C.navy}}>{p.clientName||p.customer?.name||"Unnamed"}</div>
    ...
    <div style={{width:"100%",maxWidth:400}}>
      <div style={{textAlign:"center",marginBottom:32}}>
        <div style={{display:"flex",justifyContent:"center"}}>
          <img src="/Paintship W-W-Logo.png" alt="PaintShip — Richest Homes Painting Service" style={{width:280,maxWidth:"85%",height:"auto",display:"block"}}/>
        </div>
      </div>
...
// ─── PDF GENERATOR ────────────────────────────────────────────────
// ─── JSON EXPORT — for the external PaintShip Operations app ───────────
// Exports the existing project state exactly as-is (no second data model,
// no recalculation) as a downloadable .json file.
...
function generateProjectJSON(project) {
  ...
  const ref = `PS-${(project.id||"").slice(0,6).toUpperCase()}`;
  const clientSlug = slugify(project.customer?.name) || "project";
  const a = document.createElement("a");
  a.href = url; a.download = `${ref}-${clientSlug}.json`;
  document.body.appendChild(a); a.click();
  setTimeout(()=>{document.body.removeChild(a);URL.revokeObjectURL(url);},3000);
}

function generatePDF(project) {
  const legacyCheck = hasLegacyExtras(project);
  if (legacyCheck.hasLegacy) {
    alert("PDF blocked: Legacy Extras are present and are not included in the Grand Total. Migrate or remove these entries before generating the final quotation.");
    return;
  }
  const withMat=project.quoteMode==="with_material";
...
function esc2(s){ return (s||"").toString().replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;"); }
...
td{font-size:11.5px;padding:6px 8px;border-bottom:1px solid #E5E7EB}
...
function generatePurchaseListPDF(project) {
  const { rows } = buildMaterialConsumptionSummary(project);
  if (!rows.length) { alert("No purchase list data to export yet."); return; }
  const date = new Date().toLocaleDateString("en-IN",{day:"2-digit",month:"short",year:"numeric"});
  const rowsHtml = rows.map(r=>`<tr><td>${esc2(r.material)}</td><td>${esc2(r.brand||"-")}</td><td>${esc2(r.product||"-")}</td><td style="text-align:right;font-weight:700">${r.qty.toFixed(r.unit==="rolls"?0:2)} ${r.unit}</td></tr>`).join("");
  const bodyHtml = `<h1>Purchase List</h1>
<div class="sub">${esc2(project.customer?.name||"Client")} · ${date} · Vendor-ready procurement list</div>
<table><thead><tr><th>Material</th><th>Brand</th><th>Product</th><th style="text-align:right">Quantity to Order</th></tr></thead>
<tbody>${rowsHtml}</tbody></table>`;
...
export default function App() {
  ...
  useEffect(()=>{ if(user) loadAllProjects().then(setProjects); },[user]);
  useEffect(()=>{
    if(!project||!user) return;
    if(autoRef.current) clearTimeout(autoRef.current);
    autoRef.current=setTimeout(()=>doSave(true),10000);
    return ()=>clearTimeout(autoRef.current);
  },[project]);

  if(!user) return <LoginScreen onLogin={u=>{ setUser(u); if(!u.isAdmin) setProject(newProject(u)); }}/>;
  if(user.isAdmin) return <AdminScreen projects={projects} onLogout={()=>setUser(null)}/>;

  const floor=project.floors[af];
  ...
  const handleNewProject = () => {
    ...
    setProject({ ...newProject(user), id: newId });
    setAf(0);
    setAr(0);
    setNav("calc");
  };
  ...
  const doSave = async (auto = false) => {
    // BUGFIX (Infinite ⏳ Freeze): the whole body now runs inside
    // try/catch/finally. Previously an unhandled throw anywhere between here
    // and setCloudSt(...) — e.g. a rejected saveProject() call, or
    // setProjects()/JSON logic blowing up on unexpected data — left cloudSt
    // stuck on "saving"/"auto" forever because the reset line was never
    // reached. finally{} now guarantees the status always resolves, and a
    // belt-and-braces setTimeout fallback forces it back to idle even if
    // some future code path still manages to dodge the try/catch.
    setCloudSt(auto ? "auto" : "saving");

    // Fallback safety-net: no matter what happens above, never let the UI
    // stay frozen on ⏳ for more than 12s.
    const freezeGuard = setTimeout(() => setCloudSt(cur => (cur === "saving" || cur === "auto") ? "idle" : cur), 12000);
    ...
    try {
      ...
      const payload = {
        ...project,
        id: activeId,
        clientName: project?.customer?.name || project?.clientName || "Unnamed",
      ...
      setCloudSt(res?.ok ? (auto ? "auto_ok" : "saved") : "error");
    } catch (err) {
  ...
  const dupProject=async(p,e)=>{ e.stopPropagation(); const d={...JSON.parse(JSON.stringify(p)),id:uid(),createdAt:new Date().toISOString(),updatedAt:new Date().toISOString(),customer:{...p.customer,name:p.customer.name+" (Copy)"}}; const res=await saveProject(d); const saved=res.project||d; setProjects(all=>[saved,...all]); };
  const delProject=async(id,e)=>{ e.preventDefault(); e.stopPropagation(); if(!window.confirm("Delete this estimate permanently?")) return; setProjects(all=>all.filter(p=>p.id!==id)); await deleteProject(id); };

  const statusColor={ idle:C.navy, saving:C.gold, auto:"#555", auto_ok:C.teal, saved:C.green, error:C.red };
  ...
  return <div style={{maxWidth:560,margin:"0 auto",background:C.bg,minHeight:"100vh",fontFamily:"system-ui,-apple-system,sans-serif",display:"flex",flexDirection:"column"}}>
        ...
        <div style={{flex:1,padding:"0 0 20px"}}>
          ...
          {currentStepDef.id===0&&<>
            <div style={{...CARD,marginTop:0}}>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:10}}>
                ...
                <Inp label="Mobile" value={project.customer.mobile} onChange={v=>upCust("mobile",v)} placeholder="+91..." type="tel"/>
          ...
          {currentStepDef.id===4&&<>
            ...
            {paintTab==="interior"&&showInterior&&(()=>{
              ...
              return <>
                ...
                {pFloors.length>1&&<div style={{display:"flex",gap:6,overflowX:"auto",paddingBottom:4,marginBottom:12}}>
                  {pFloors.map((fl,fi)=>(
                    <button key={fl.id} onClick={()=>setPaintFloorIdx(fi)}
                ...
                {(()=>{
                  ...
                  return <div key={r.id} style={{background:C.white,
            ...
            {paintTab==="exterior"&&(()=>{
              ...
              const configuredTotals = calcExteriorConfiguredTotals(extElevations, ec, project.quoteMode);
          ...
          {currentStepDef.id===6&&<>
            ...
            {/* PDF Logo — permanently the official PaintShip brand mark.
                Upload/Remove controls removed; project.pdfLogo is no longer
                written to or read for display here (field left in the data
                model for backward compatibility, simply unused now). */}
            ...
            <MaterialConsumptionSummary project={project}/>
      ...
      {nav==="projects"&&(()=>{ const filteredProjects=projects.filter(p=>{ const q=(projSearch||"").toLowerCase(); const name=(p.clientName||p.customer?.name||"").toLowerCase(); const loc=(p.location||p.customer?.location||"").toLowerCase(); return !q||name.includes(q)||loc.includes(q); }); return <div style={{padding:18}}>
        ...
        {filteredProjects.length===0
          ...
          :filteredProjects.map(p=>{
            const pt=projectTotals(p);const rooms=p.floors?.reduce((s,f)=>s+f.rooms.length,0)||0;const wm=p.quoteMode==="with_material";
            return <div key={p.id} onClick={()=>{ const migrated={...p,floors:p.floors.map(fl=>({...fl,rooms:fl.rooms.map(r=>migrateRoom(r))})),exterior:(p.exterior||[]).map(el=>migrateElevation(el))}; setProject(migrated);setAf(0);setAr(0);setNav("calc"); }} style={{...CARD,cursor:"pointer",marginBottom:10}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                <div style={{flex:1}}>
                  <div style={{fontSize:15,fontWeight:800,color:C.navy}}>{p.clientName||p.customer?.name||"Unnamed"}</div>
                ...
                <div style={{textAlign:"right",display:"flex",flexDirection:"column",gap:5,marginLeft:10}}>
                  <div style={{fontSize:15,fontWeight:800,color:C.orange}}>₹{(p.grandTotal||p.totalAmount||(wm?pt.totalIncl:pt.totalExcl)).toFixed(0)}</div>
                  <div style={{fontSize:11,color:C.gray}}>{pt.net.toFixed(0)} sf</div>
    ...
    {/* Bottom bar */}
    <div style={{position:"fixed",bottom:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:560,background:C.white,borderTop:`1px solid ${C.border}`,zIndex:100,boxShadow:"0 -4px 20px rgba(15,30,60,0.12)",paddingBottom:"env(safe-area-inset-bottom,0px)"}}>
      {nav==="calc"&&(()=>{
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,padding:"14px 16px 10px"}}>
          <div style={{background:`linear-gradient(135deg,${C.navy},${C.navyL})`,borderRadius:13,padding:"14px 16px",border:`1px solid rgba(232,160,32,0.3)`,minHeight:78,display:"flex",flexDirection:"column",justifyContent:"center",gap:3}}>
            <div style={{fontSize:9,color:"rgba(255,255,255,0.75)",fontWeight:700,letterSpacing:"0.06em"}}>🏆 {isJoinery||isWallpaperMT||isTextureMT?"SECTION TOTAL":"GRAND TOTAL"}</div>
            <div style={{fontSize:19,fontWeight:900,color:C.orange}}>₹{btmTotal.toFixed(0)}</div>
            <div style={{fontSize:9,color:"rgba(255,255,255,0.5)"}}>{isJoinery?"Wood, Metal & Joinery":isWallpaperMT?"Wallpaper":isTextureMT?"Texture":isExteriorMT?"Exterior":"Interior"}</div>
          </div>
          <div style={{background:C.white,borderRadius:13,padding:"14px 16px",border:`1px solid ${C.border}`,minHeight:78,display:"flex",flexDirection:"column",justifyContent:"center",gap:3}}>
            <div style={{fontSize:9,color:C.gray,fontWeight:700,letterSpacing:"0.06em"}}>📐 TOTAL AREA</div>
            <div style={{fontSize:19,fontWeight:900,color:C.navy}}>{btmArea.toFixed(1)} sf</div>
            <div style={{fontSize:9,color:"#aaa"}}>{isJoinery?`Items: ${doorWindowCalc.area.toFixed(0)}sf · Finish: ${polishCalc.net.toFixed(0)}sf`:isWallpaperMT?`Wallpaper: ${totals.wallpaper.area.toFixed(0)}sf`:isTextureMT?`Texture: ${totals.texture.area.toFixed(0)}sf`:isExteriorMT?`Ext: ${totals.exterior.area.toFixed(0)}sf`:`Int: ${totals.interior.area.toFixed(0)}sf`}</div>
          </div>
        </div>
        {/* Wizard navigation buttons */}
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10,padding:"8px 16px 14px"}}>
          <button
            onClick={()=>setWizStep(s=>Math.max(0,s-1))}
            disabled={wizIsFirst}
            style={{padding:"14px 0",border:`1.5px solid ${wizIsFirst?C.border:C.navy}`,borderRadius:12,fontSize:12,fontWeight:700,cursor:wizIsFirst?"not-allowed":"pointer",background:wizIsFirst?"#F8FAFC":C.white,color:wizIsFirst?C.gray:C.navy,opacity:wizIsFirst?0.45:1,transition:"all 0.2s"}}>← Prev</button>
          <button
            onClick={async () => {
              try {
                await doSave();
              } catch (e) {
                console.error("Save button handler failed:", e);
              }
            }}
            style={{padding:"14px 0",border:"none",borderRadius:12,fontSize:11,fontWeight:700,cursor:"pointer",background:statusColor[cloudSt]||C.navy,color:"#fff",transition:"background 0.3s"}}>{statusText[cloudSt]||"☁ Save"}</button>
          <button
            onClick={()=>{ if(wizIsLast){setShowQuote(true);}else{setWizStep(s=>Math.min(wizVisibleSteps.length-1,s+1));} }}
            style={{padding:"14px 0",border:"none",borderRadius:12,fontSize:12,fontWeight:800,cursor:"pointer",background:wizIsLast?`linear-gradient(135deg,${C.gold},#F5B942)`:`linear-gradient(135deg,${C.navy},${C.navyL})`,color:wizIsLast?C.navy:"#fff",transition:"all 0.2s"}}>{wizIsLast?"Review ⚡":"Next →"}</button>
        </div>
        </>;
      })()}
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",borderTop:`1px solid ${C.border}`,background:C.navy}}>
        {[{id:"calc",icon:"📐",label:"Measure"},{id:"projects",icon:"📁",label:"Estimates"}].map(n=>(
          <button key={n.id} onClick={()=>setNav(n.id)} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:2,padding:"10px 4px",border:"none",background:"none",fontSize:10,fontWeight:700,color:nav===n.id?C.gold:"rgba(255,255,255,0.4)",cursor:"pointer",textTransform:"uppercase"}}>
            <span style={{fontSize:20}}>{n.icon}</span>{n.label}
            {nav===n.id&&<div style={{width:24,height:2,background:C.gold,borderRadius:1,marginTop:1}}/>}
          </button>
        ))}
      </div>
    </div>
    ...
    {showQuote&&(()=>{
      ...
      // not by project.scope. This ensures a polish-only project shows only the Polish
      // section (Interior/Exterior net area is 0 when unused), while a project with data
      // in multiple sections shows all of them.
      const qShowInterior = intNet>0;
      const qShowExterior = extNet>0;
      const qShowPolish = polishCalc.net>0;
      const qShowDoorWindow = (project.doorWindowItems||[]).length>0;
      const qShowWallpaper = (project.wallpaperItems||[]).length>0;
      const qShowTX2Texture = calcTexture(project.TX2_textureItems||[]).total>0;
      return <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.55)",zIndex:200,display:"flex",alignItems:"flex-end",justifyContent:"center"}} onClick={()=>setShowQuote(false)}>
      <div onClick={e=>e.stopPropagation()} style={{background:C.white,borderRadius:"20px 20px 0 0",padding:"22px 18px 36px",width:"100%",maxWidth:480,maxHeight:"90vh",overflowY:"auto"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
          <div><div style={{fontSize:20,fontWeight:800,color:C.navy}}>Quote Summary</div><div style={{fontSize:12,color:"#aaa"}}>{project.customer.name||"—"} · {project.customer.location||""}</div></div>
        ...
        <div style={{display:"flex",gap:8,marginBottom:14,flexWrap:"wrap"}}>
          <span style={{background:project.projectType==="fresh"?C.greenL:C.orangeL,color:project.projectType==="fresh"?C.green:C.orange,fontSize:11,fontWeight:700,borderRadius:20,padding:"4px 12px"}}>{project.projectType==="fresh"?"🎨 Fresh":"🔄 Repaint"}</span>
          <span style={{background:withMat?C.navy:"#F8FAFC",color:withMat?"#fff":C.gray,fontSize:11,fontWeight:700,borderRadius:20,padding:"4px 12px"}}>{withMat?"💎 With Material":"📐 Measurement Only"}</span>
        </div>

        {(()=>{
          const lcTop = hasLegacyExtras(project);
          if (!lcTop.hasLegacy) return null;
          return <div style={{background:"#FFF7ED",border:"2px solid #F59E0B",borderRadius:12,padding:"12px 14px",marginBottom:14}}>
            ...
            <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
              <span style={{fontSize:11,fontWeight:700,color:"#B45309",background:"#FDE68A",borderRadius:20,padding:"3px 10px"}}>Doors/Windows: {lcTop.legacyDoorCount}</span>
              <span style={{fontSize:11,fontWeight:700,color:"#B45309",background:"#FDE68A",borderRadius:20,padding:"3px 10px"}}>Wallpaper: {lcTop.legacyWallpaperCount}</span>
              <span style={{fontSize:11,fontWeight:700,color:"#B45309",background:"#FDE68A",borderRadius:20,padding:"3px 10px"}}>Texture: {lcTop.legacyTextureCount}</span>
            </div>
          </div>;
        })()}

        {(()=>{
          const qt = getProjectServiceTotals(project);
          const qlc = hasLegacyExtras(project);
          const modules = [
            { label:"Interior", active: (qt.interior?.net||0)>0 },
            { label:"Exterior", active: (qt.exterior?.net||0)>0 },
            { label:"Joinery", active: ((qt.doorwindow?.total||0)+(qt.polish?.total||0))>0 },
            { label:"Wallpaper", active: (qt.wallpaper?.total||0)>0 },
            { label:"Texture", active: (qt.texture?.total||0)>0 },
          ].filter(m=>m.active);

          const checks = [
            { label:"Client name", pass: !!(project.customer && project.customer.name) },
            { label:"Total area", pass: (qt.grandArea||0)>0 },
            { label:"Grand total", pass: (qt.grandTotal||0)>0 },
            { label:"No legacy extras", pass: !qlc.hasLegacy },
          ];
          const allPass = checks.every(c=>c.pass);
          const ready = !qlc.hasLegacy && (qt.grandTotal||0)>0 && (qt.grandArea||0)>0;
        ...
        {qShowInterior&&project.floors.map(fl=>(
          <div key={fl.id} style={{marginBottom:14}}>
            ...
            {fl.rooms.map(r=>{ const rc2=calcRoom(r);const rpkg=PACKAGES[r.package]||PACKAGES.premium;const fin=r.finishing||{};const pName=withMat?getProductName(r.brand,r.package,"interior"):"";
              return <div key={r.id} style={{background:"#FAFAFA",borderRadius:12,padding:"12px 14px",marginBottom:8,border:`1px solid ${C.border}`}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:6}}>
                  <div>
                    ...
                    {withMat&&<div style={{display:"flex",gap:6,marginTop:4,alignItems:"center",flexWrap:"wrap"}}>
                      ...
                      {project.includeBrand&&<div style={{display:"flex",alignItems:"center",gap:3,background:"#F0F0F0",borderRadius:20,padding:"2px 7px"}}><div style={{borderRadius:3,overflow:"hidden"}}><BrandLogo id={r.brand} size={13}/></div><span style={{fontSize:9,fontWeight:700,color:"#555"}}>{getBrandName(r)}</span></div>}
                      {pName&&<span style={{fontSize:9,color:C.orange,fontWeight:600}}>{pName}</span>}
                    </div>}
                  </div>
                  <span style={{fontSize:14,fontWeight:800,color:C.navy}}>{rc2.net.toFixed(1)} sf</span>
                </div>
                ...
                {withMat&&<div style={{borderTop:`1px solid ${C.border}`,paddingTop:6,marginTop:6}}>
                  {Object.entries(fin).filter(([,f])=>f.on).map(([k,f])=>{ const a=f.useRoom?rc2.net:(f.area||0);const cost=(f.rate||0)*(f.coats||1)*a+(k==="wallpaper"?(f.installRate||0)*a:0);
                    return <div key={k} style={{display:"flex",justifyContent:"space-between",fontSize:11,padding:"2px 0",color:"#888"}}><span>{FIN_ICONS[k]} {f.type||k} ({f.coats||1}×)</span><span style={{fontWeight:600}}>₹{cost.toFixed(0)}</span></div>; })}
                </div>}
        ...
        {qShowExterior&&(()=>{
          // Canonical source only — no independent recalculation. `totals` here is the
          // existing getProjectServiceTotals(project) call already made at component top
          // (App-level `totals` variable); exteriorBreakdown comes from the same call.
          const exteriorSummary = totals.exterior;
          const exteriorBreakdown = totals.exteriorBreakdown || [];
          // Global manual-area (useRoom:false) layers are charged once by
          // calcExteriorConfiguredTotals, to the first default (non-customized) elevation.
          // Mirror that allocation here so displayed detail rows sum to exteriorSummary.material.
          ...
          return <div style={{background:"#FAFAFA",borderRadius:12,padding:"12px 14px",marginBottom:10,border:`1px solid ${C.border}`}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
              <div style={{fontSize:13,fontWeight:800,color:C.teal}}>🏗 Exterior</div>
              <span style={{fontSize:12,fontWeight:800,color:C.navy}}>{exteriorSummary.area.toFixed(1)} sf</span>
            </div>
            ...
            {exteriorBreakdown.map(entry=>{
              ...
              const eProductQ=getProductName(cfg.brand,cfg.package,"exterior");
              ...
              const finRows=LAYER_ORDER.map(key=>{
                ...
                if(!layer||layer.on!==true) return null; // never render disabled layers
                if(layer.useRoom===false&&!showManualLayers) return null; // already charged elsewhere
              ...
              return <div key={entry.id} style={{background:C.white,borderRadius:10,padding:"10px 12px",marginBottom:8,border:`1px solid ${C.border}`}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:6}}>
                  <div>
                    <div style={{fontSize:12,fontWeight:800,color:C.navy}}>{entry.name}</div>
                    <span style={{fontSize:9,fontWeight:700,padding:"2px 8px",borderRadius:20,marginTop:2,display:"inline-block",
                      background:usingDefault?"#EEF2F6":C.orangeL,color:usingDefault?C.gray:C.orange}}>
                      {usingDefault?"Default Paint":"Customized"}
                    </span>
                  </div>
                  <span style={{fontSize:12,fontWeight:800,color:C.orange}}>₹{(entry.total||0).toFixed(0)}</span>
                </div>
                {withMat&&<div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:6,alignItems:"center"}}>
                  ...
                  <div style={{display:"flex",alignItems:"center",gap:4,background:"#F0F0F0",borderRadius:20,padding:"2px 8px"}}><div style={{borderRadius:3,overflow:"hidden"}}><BrandLogo id={cfg.brand} size={13}/></div><span style={{fontSize:9,fontWeight:700,color:"#555"}}>{eBrandQ}</span></div>
                  {eProductQ&&<span style={{fontSize:9,color:C.orange,fontWeight:600}}>{eProductQ}</span>}
                </div>}
                {withMat&&finRows.length>0&&<div style={{borderTop:`1px dashed ${C.border}`,paddingTop:6,marginBottom:6}}>
                  {finRows.map(fr=>(
        ...
        {qShowPolish&&(()=>{
          ...
          return <div style={{background:"#FAFAFA",borderRadius:12,padding:"12px 14px",marginBottom:12,border:`1px solid ${C.purple}22`}}>
            <div style={{fontSize:13,fontWeight:800,color:C.navy,marginBottom:8}}>🪟 Polish / Enamel</div>
            {polishItemsQ.map(it=>{ const c=calcPolishItem(it); const fin=POLISH_FINISH_TYPES.find(f=>f.id===it.finishId)||POLISH_FINISH_TYPES[0];
              return <div key={it.id} style={{padding:"5px 0",borderBottom:`1px solid ${C.border}`}}>
                <div style={{display:"flex",justifyContent:"space-between",fontSize:11}}>
            ...
            <div style={{display:"flex",justifyContent:"space-between",marginTop:8,paddingTop:4}}>
              ...
              <span style={{fontSize:14,fontWeight:800,color:C.purple}}>₹{polishCalc.total.toFixed(0)}</span>
            </div>
          </div>;
        })()}
        {(()=>{
          const lc = hasLegacyExtras(project);
          ...
          return <div style={{background:"#FFF7ED",border:"2px solid #F59E0B",borderRadius:12,padding:"12px 14px",marginBottom:14}}>
            ...
            {lc.legacyDoorCount>0 && (()=>{
              const dwT=calcDWTotals(project.dwItems);
              const finLabel=it=>DW_FINISH_TYPES.find(f=>f.id===it.finish)?.label||(it.customFinish||it.finish);
              return <div style={{background:"#fff",borderRadius:10,padding:"10px 12px",marginBottom:8,border:"1px solid #FDE68A"}}>
                <div style={{fontSize:12,fontWeight:800,color:"#92400E",marginBottom:6}}>🚪 Legacy Door & Window</div>
                {project.dwItems.map(it=>{ const c=calcDWItem(it); return (
            ...
            {lc.legacyWallpaperCount>0 && (()=>{
              const wpT=calcWPTotals(project.wpItems);
              return <div style={{background:"#fff",borderRadius:10,padding:"10px 12px",marginBottom:8,border:"1px solid #FDE68A"}}>
                <div style={{fontSize:12,fontWeight:800,color:"#92400E",marginBottom:6}}>🖼 Legacy Wallpaper</div>
                {project.wpItems.map(it=>{ const c=calcWPItem(it); return (
            ...
            {lc.legacyTextureCount>0 && (()=>{
              const txT=calcTextureTotals(project.textureItems);
              return <div style={{background:"#fff",borderRadius:10,padding:"10px 12px",border:"1px solid #FDE68A"}}>
                <div style={{fontSize:12,fontWeight:800,color:"#92400E",marginBottom:6}}>🏔 Legacy Texture & Decorative Finish</div>
                {project.textureItems.map(it=>{ const c=calcTextureItem(it); return (
                  <div key={it.id} style={{padding:"5px 0",borderBottom:"1px solid #FDE68A"}}>
                    <div style={{display:"flex",justifyContent:"space-between",fontSize:11}}>
        ...
        {qShowDoorWindow&&(()=>{
          ...
          const dwCalcQ=calcDoorWindow(dwItemsQ);
          return <div style={{background:"#FAFAFA",borderRadius:12,padding:"12px 14px",marginBottom:12,border:`1px solid ${C.border}`}}>
            <div style={{fontSize:13,fontWeight:800,color:C.navy,marginBottom:8}}>🚪 Door & Window</div>
            {dwItemsQ.map(it=>{ const c=calcDoorWindowItem(it); const fin=DW2_FINISH_TYPES.find(f=>f.id===it.finishType)||DW2_FINISH_TYPES[0];
              return <div key={it.id} style={{padding:"5px 0",borderBottom:`1px solid ${C.border}`}}>
                <div style={{display:"flex",justifyContent:"space-between",fontSize:11}}>
        ...
        {qShowWallpaper&&(()=>{
          ...
          const wpCalcQ=calcWallpaper(wpItemsQ);
          return <div style={{background:"#FAFAFA",borderRadius:12,padding:"12px 14px",marginBottom:12,border:`1px solid ${C.purple}22`}}>
            <div style={{fontSize:13,fontWeight:800,color:C.navy,marginBottom:8}}>🖼 Wallpaper</div>
            {wpItemsQ.map(it=>{ const c=calcWallpaperItem(it);
              return <div key={it.id} style={{padding:"5px 0",borderBottom:`1px solid ${C.border}`}}>
                <div style={{display:"flex",justifyContent:"space-between",fontSize:11}}>
        ...
        {qShowTX2Texture&&(()=>{
          ...
          const txCalcQ=calcTexture(txItemsQ);
          return <div style={{background:"#FAFAFA",borderRadius:12,padding:"12px 14px",marginBottom:12,border:`1px solid ${C.purple}22`}}>
            <div style={{fontSize:13,fontWeight:800,color:C.navy,marginBottom:8}}>🧱 Texture</div>
            {txItemsQ.map(it=>{ const c=TX2_calcTextureItem(it);
              return <div key={it.id} style={{padding:"5px 0",borderBottom:`1px solid ${C.border}`}}>
                <div style={{display:"flex",justifyContent:"space-between",fontSize:11}}>
